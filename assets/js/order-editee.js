...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var tempTableCon = document.getElementById("products");
  var tempProductCount = tempTableCon.getElementsByTagName("tr").length;
  console.log(tempProductCount);
  for (i = 1 ; i < tempProductCount ; i++) {
      const searchInput = document.getElementById('searchInput_'+i);
      console.log(searchInput);
      const searchResults = document.getElementById('searchResults_'+i);
      const productSelect = document.getElementById('selectProduct_'+i);
      ...
      if (searchInput && searchResults && productSelect) {
        searchInput.addEventListener('input', function() {
          ...
          if (!isSearchInProgress) {
            ...
            fetch('https://anas.safwa.xyz/public/api/search-edit?query=' + searchInputValue)
              .then(function(response) {
                return response.json();
              })
              .then(function(response) {
...
function addFun() {
  ...
  var rowcontent = `
    <tr>
      ...
      <td>
        <input type="text" id="searchInput_${productCount}" placeholder="البحث عن مادة" class="form-control">
...
/////////////////// for delete 
function removeRow(button, orderId, productId) {
  ...
  fetch(`/order/${orderId}/product/${productId}`, {
    method: 'DELETE',
    headers: {
  ...
  .then(response => {
    if (response.ok) {
      // Product successfully removed from the order
      console.log('Product removed from the order.');
    } else {
      // Handle the error case
      console.error('Failed to remove product from the order.');
    }
  })
  .catch(error => {
...
if (searchInput && searchResults && customerSelect) {
  searchInput.addEventListener('input', function() {
    ...
    if (!isSearchInProgress) {
      ...
      fetch('https://anas.safwa.xyz/public/api/searchCustmer-edit?query=' + searchInputValue)
        .then(function(response) {
          return response.json();
        })
        .then(function(response) {
...
if (searchInputSupplier && searchResultsSupplier && supplierSelect) {
  searchInputSupplier.addEventListener('input', function() {
    ...
    if (!isSearchInProgressSupplier) {
      ...
      fetch('https://anas.safwa.xyz/public/api/searchSupplier-edit?query=' + searchInputValueSupplier)
        .then(function(response) {
          return response.json();
        })
        .then(function(response) {
