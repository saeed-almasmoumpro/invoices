...
customerDropdown.addEventListener('change', function() {
  ...
  codeInput.value = selectedOption.getAttribute('data-code');
    ...
    // Update values based on selected option
    var selectedOption = customerDropdown.options[customerDropdown.selectedIndex];
...
function addFun() {
    ...
    // Call the function to populate the existing products
    populateExistingProducts(productSelect);
...
function populateExistingProducts(productSelect) {
    // Perform AJAX request to fetch the existing products
    fetch('/api/get-existing-products')
        .then(function (response) {
            return response.json();
        })
        .then(function (response) {
           ...
           // Create and append new options based on the existing products
            response.forEach(function (product) {
                const option = document.createElement('option');
                option.value = product.id;
                option.textContent = product.name;
...
function updateTotalPrice(newRow) {
    const productCount = newRow.querySelector('input').id.split('_')[1];
